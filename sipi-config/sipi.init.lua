...
function pre_flight(prefix,identifier,cookie)
    ...
    if prefix == 'imgrep' then
        ...
        -- Log the received prefix and identifier for debugging
        server.log('pre_flight: prefix=' .. tostring(prefix) .. ', identifier=' .. tostring(identifier), server.loglevel.LOG_ERR)
        
        if config.prefix_as_path then
            filepath = config.imgroot .. '/' .. prefix .. '/' .. identifier
        else
            -- Ensure we use the full identifier path, even if it contains slashes
            filepath = config.imgroot .. '/' .. identifier
        end
        
        -- Log the constructed filepath
        server.log('pre_flight: filepath=' .. filepath, server.loglevel.LOG_ERR)
    end
