...
# The api/celery containers run as the image's `archetype` user and write to
# storage/uploads_tmp/ (image-upload chunks), storage/media/uploads/ (the served
# JP2s) and storage/media/{carousel,partners}/ (carousel images and partner
# logos, served by nginx; see nginx.conf). Until those directories exist and
# are writable by that user, uploads fail. The chown runs inside the api image
# (as root) so no host sudo is needed.

# One-time setup: create the upload storage dirs and chown them to the service user
setup-upload-storage:
    docker compose run --rm --no-deps -u root api sh -c 'mkdir -p /app/storage/uploads_tmp /app/storage/media/uploads /app/storage/media/carousel /app/storage/media/partners && chown -R archetype:archetype /app/storage/uploads_tmp /app/storage/media/uploads /app/storage/media/carousel /app/storage/media/partners'
